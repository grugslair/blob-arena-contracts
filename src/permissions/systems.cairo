...
use dojo::{world::IWorldDispatcher, model::ModelStorage};
...
impl PermissionsImpl of Permissions {
    fn has_admin_permission(self: @IWorldDispatcher, requester: ContractAddress) -> bool {
        self.get_permission(requester, Role::Admin)
    }

    fn has_role_permission(
        self: @IWorldDispatcher, requester: ContractAddress, role: Role,
    ) -> bool {
        self.get_permission(requester, role)
    }

    fn has_a_role_permission(
        self: @IWorldDispatcher, requester: ContractAddress, mut roles: Array<Role>,
    ) -> bool {
        loop {
    ...
    fn has_permission(self: @IWorldDispatcher, requester: ContractAddress, role: Role) -> bool {
        self.get_permission(requester, role) || self.has_parent_role_permission(requester, role)
    }


    fn caller_has_permission(self: @IWorldDispatcher, role: Role) -> bool {
        self.has_permission(get_caller_address(), role)
    }

    fn assert_has_permission(self: @IWorldDispatcher, requester: ContractAddress, role: Role) {
        if !self.has_permission(requester, role) {
            panic!("User does not have {} permission", Into::<_, ByteArray>::into(role));
        }
    }

    fn assert_caller_has_permission(self: @IWorldDispatcher, role: Role) {
        if !self.caller_has_permission(role) {
            panic!("Caller does not have {} permission", Into::<_, ByteArray>::into(role));
        }
    }

    fn assert_caller_is_admin(self: @IWorldDispatcher) {
