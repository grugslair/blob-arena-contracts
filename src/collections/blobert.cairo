...
mod blobert_actions {
    ...
    impl ICollectionImpl of ICollection<ContractState> {
        fn get_owner(world: @WorldStorage, token_id: u256) -> ContractAddress {
            let dispatcher = get_erc271_dispatcher();
            IERC721DispatcherTrait::owner_of(dispatcher, token_id)
        }
        fn get_health(world: @WorldStorage, token_id: u256) -> u8 {
            let traits = world.get_blobert_traits(token_id);
            world.get_blobert_health(traits)
        }
        fn get_stats(world: @WorldStorage, token_id: u256) -> Stats {
            let traits = world.get_blobert_traits(token_id);
            world.get_blobert_stats(traits)
        }
        fn get_item_ids(world: @WorldStorage, token_id: u256) -> Span<felt252> {
            let traits = world.get_blobert_traits(token_id);
            world.get_blobert_item_ids(traits).span()
        }
        fn has_attack(
            world: @WorldStorage, token_id: u256, item_id: felt252, attack_id: felt252
        ) -> bool {
            let traits = world.get_blobert_traits(token_id);
