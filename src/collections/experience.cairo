...
#[generate_trait]
impl ExperienceStatsImpl<+BlobertStore> of ExperienceStatsTrait<> {
    fn increase_stats(
        ref self: IWorldDispatcher, token: u256, player: ContractAddress, stats_increase: UStats,
    ) {
    ...
    fn remove_overflowing_stats(ref self: IWorldDispatcher, token: u256, player: ContractAddress) {
        let mut storage = self.local_store();
