    ...
    impl PvPActionsImpl of IPvPCombatActions<ContractState> {
        ...
        fn reveal_attack(
            ...
            if hash == commitment {
                ...
                let other_planned_attack = world.get_planned_attack(other_combatant);
                if other_planned_attack.attack.is_non_zero() {
                    let combatants_span: Span<u128> = combatants.into();
                    let hashs = world.get_salts_hash_state(combat.id);
                    let planned_attacks = array![planned_attack, other_planned_attack].span();
                    let states = world
                        .run_round(
                            combatants, planned_attacks.try_into().unwrap(), combat.round, hashs
                        );
