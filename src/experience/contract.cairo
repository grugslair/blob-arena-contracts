...
trait IExperienceActions<TContractState> {
    ...
    fn allocate_stats(
        ref self: TContractState, collection: ContractAddress, token: u256, stats: UStats,
    );
    fn remove_overflowing_stats(ref self: TContractState, collection: ContractAddress, token: u256);
}
