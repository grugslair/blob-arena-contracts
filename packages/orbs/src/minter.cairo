...
mod orbs_minter {
    ...
    impl IOrbMinterImpl of IOrbMinter<ContractState> {
        fn roll(ref self: ContractState, to: ContractAddress, chances_permille: [u32; 4]) -> u256 {
            ...
            let (action_id, rarity) = self.get_random_action(chances_permille, randomness);
            self.orb_contract.read().mint(to, action_id, rarity);
        }
    }
    ...
    impl PrivateImpl of PrivateTrait {
        fn get_random_action(
            ...
            if value < chance.into() {
                let number = seed.get_final_value(self.common_actions_amount.read());
                return (self.common_actions.read(number), Rarity::Common);
            }
            chance += rare;
            if value < chance.into() {
                let number = seed.get_final_value(self.rare_actions_amount.read());
                return (self.rare_actions.read(number), Rarity::Rare);
            }
            chance += epic;
            if value < chance.into() {
                let number = seed.get_final_value(self.epic_actions_amount.read());
                return (self.epic_actions.read(number), Rarity::Epic);
            }
            chance += legendary;
            if value < chance.into() {
                let action_n = seed.get_final_value(self.legendary_actions_amount.read());

                return (self.legendary_actions.read(action_n), Rarity::Legendary);
            }
