...
#[starknet::interface]
pub trait IOrbUser<TContractState> {
...
pub impl OrbImpl of OrbTrait {
    fn try_use_owners_charge_cost(
        ...
        if (ERC721ABIDispatcher { contract_address: self }.owner_of(token_id) == user) {
            IOrbUserDispatcher { contract_address: self }.try_use_charge_cost(token_id)
        } else {
            None
        }
    }
...
pub mod orb {
    ...
    use super::{IOrb, IOrbAdmin, IOrbUser, Role, downcast_id};
    ...
    #[abi(embed_v0)]
    impl IOrbUserImpl of IOrbUser<ContractState> {
